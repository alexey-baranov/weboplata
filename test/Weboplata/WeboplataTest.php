...
class WeboplataTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->object = new \Zend_Rest_Client("http://localhost/weboplata/public/1.0.php");
    }
    ...
    {
        /** @var \Zend_Rest_Client_Result $result */
        $result= $this->object->add("789-И", 1, (new \DateTime)->format('Y-m-d\TH:i:s'), "unit-test", 8, (new \DateTime)->getTimestamp())
            ->get();
    ...
    {
        /** @var \Zend_Rest_Client_Result $result */
        $result= (new Weboplata)->add("789-И", 1, (new \DateTime)->format('Y-m-d\TH:i:s'), "unit-test", 8, (new \DateTime)->getTimestamp()+1);

        $this->assertEquals(Weboplata::OK, $result);
    }

/*    public function testIsAerotel($AccountNumber)
    {
        $weboplata= new Weboplata;
